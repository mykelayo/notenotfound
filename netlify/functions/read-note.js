...
exports.handler = async function (event) {
  ...
  try {
    ...
    raw = results[0].result; // the stored string, or null if not found / expired
  } catch (err) {
  ...
  // null = key doesn't exist (never created, already read, or TTL expired)
  ...
  // Parse the JSON string → note object
  let note;
  try {
    note = JSON.parse(raw);
  } catch (err) {
  ...
  // Must be a plain object with a content field
  if (!note || typeof note !== "object" || Array.isArray(note) || typeof note.content !== "string") {
    console.error("[read-note] Note has wrong shape. keys:", Object.keys(note || {}), "isArray:", Array.isArray(note));
    return { statusCode: 500, headers: CORS, body: JSON.stringify({ error: "Note data is invalid. Please flush your database and redeploy — old notes are incompatible." }) };
  }
  ...
  // If delete fails → note stays but TTL will clean it up, content still returned
  ...
  try {
    ...
    console.log("[read-note] Note", id.trim(), "read and destroyed successfully.");
  } catch (err) {
    console.error("[read-note] DEL failed (non-fatal — TTL will clean up):", err.message);
    // Don't fail the request — content was successfully decrypted, return it
  }
